...
class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
		...
		$class_names = $class_names ? ' class="' . esc_attr( $class_names ) . 'menu-item menu-item-type-taxonomy menu-item-object-product_cat four-megamenu first menu-item-18374 widgets_dropdown  drop_to_right submenu_default_width columns1"' : '';
		...
		$prod = get_loop_prod($item->custom,$item->custom2);
		if($prod){
			$content = '<ul class="aws-sub-menu aws_prod_list" style="display:none;">';
			foreach ($prod->posts as $p) {
				$details = wc_get_product($p->ID);
				$price = $details->get_price();
				$image = get_the_post_thumbnail_url($p->ID);
				$content .= '<li class="aws_li_prod">';
				$content .= '<a href="'.$p->guid.'">';
				$content .= '<div class="thumbail">';
				$content .= '<img  width="100" height="100" src="'.$image.'" class="aws-thumbnail product"></div>';
				$content .= '<div class="aws-data">';
				$content .= '<span class="aws-product-title">'.$p->post_title.'</span>';
				$content .= '<p class="aws-product"><span class="aws-price">$ '.$price.'</span></span></p>
				</div></a></li>';
			}
			$content .= '</ul>';
			$item_output .= $content;
		...
		//ToDo
		// ya cree la estructura mas o menos, solo faltaria agregar los estilso que esos pueden ir en el archivo de clase que se agrega al plugin
		
		}
		
		//$item_output .= '<span>'.$prod->post->post_title.'</span>';
